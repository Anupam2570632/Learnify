...
const StudentClassDetails = () => {
  ...
  const { data: quizes = [] } = useQuery({
    queryKey: ["quizes", id],
    queryFn: async () => {
      const res = await axiosSecure.get(`/quizes/${id}`);
      return res.data;
    },
  });
  ...
  return (
    <div>
      <button
        onClick={openModal}
        className="bg-blue-600 my-6 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded"
      >
        Evaluate Class
      </button>
      {isModalOpen && (
      ...
      <div>
        <div>
          <div className="overflow-x-auto">
            <h1 className="text-center py-4 text-2xl font-bold">All Quizes</h1>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {quizes.map((quiz) => (
                  <tr key={quiz._id}>
                    ...
                    <td>{quiz.quizTime}</td>
                    <td>{JSON.parse(quiz.questionJson).length}</td>
                    <td>
                      <Link
                        to={`/quiz/${quiz._id}`}
                      >
                        <button className="flex items-center px-4 py-2 text-sm font-medium text-blue-600 bg-blue-100 rounded-md hover:bg-blue-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                          Start Quiz
                        </button>
                      </Link>
                    </td>
                  ...
                  <tr key={assignment._id}>
                    ...
                    <td>
                      <button
                        ...
                        className="flex items-center px-4 py-2 text-sm font-medium text-blue-600 bg-blue-100 rounded-md hover:bg-blue-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
                      >
